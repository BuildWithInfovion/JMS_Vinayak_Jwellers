...
const ProductSelector = ({
  ...
  const getAvailability = (product) => {
    ...
    const isAvailable = isBulk ? product.weight > 0 : product.stock > 0;
    const stockLabel = isBulk
      ? `Weight: ${product.weight.toFixed(3)}g`
      : `Stock: ${product.stock} pcs`;
    return { isAvailable, stockLabel };
  };
        ...
        <ul className="divide-y divide-gray-200">
          {products.length === 0 ? (
            ...
            products.map((product) => {
              const { isAvailable, stockLabel } = getAvailability(product);
              return (
                <li
                  key={product._id}
                ...
                >
                  <div>
                    ...
                    <p className={`text-sm ${isAvailable ? "text-gray-500" : "text-red-400"}`}>
                      {isAvailable ? stockLabel : "Out of Stock"}
                    </p>
                  </div>
                  <button
